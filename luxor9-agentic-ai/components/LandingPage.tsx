...
import { motion, useScroll, useTransform, useSpring, AnimatePresence, useMotionValue, useMotionTemplate, useInView } from 'framer-motion';
import { Canvas, useFrame } from '@react-three/fiber';
import { Float, Stars, MeshDistortMaterial, Sphere, TorusKnot, Sparkles as Sparkles3D, Icosahedron, Box as Box3D } from '@react-three/drei';
import * as THREE from 'three';
import { 
  ArrowRight, Terminal, Cpu, Shield, Zap, Globe, 
  Layers, ChevronRight, Play, Box, 
  Activity, CheckCircle2, 
  Code, Server, BrainCircuit,
  Database, Eye, Fingerprint, Key,
  Lock, Share2, MousePointer2, Command, Hexagon, Sparkles, Check
} from 'lucide-react';

// Fix for R3F Intrinsic Elements types: types suppressed via nocheck due to environment configuration limits
    ...
    if (this.state.hasError) {
      return (
        <div className="absolute inset-0 flex items-center justify-center opacity-20">
            <div className="w-64 h-64 bg-gradient-to-tr from-amber-500/20 to-transparent rounded-full blur-3xl"></div>
        </div>
      ); 
    }
...
const ScrollCrystal = ({ scrollYProgress }: { scrollYProgress: any }) => {
    ...
    useFrame((state, delta) => {
        if (meshRef.current) {
            ...
            // Pulse based on scroll position
            const pulse = 1 + Math.sin(state.clock.elapsedTime * 2) * 0.05 + (scroll * 0.3);
            meshRef.current.scale.setScalar(pulse);
            
            // Color shift logic could go here if we used a shader material, 
            // but for simple standard material we stick to props.
        }
    });
...
const SpotlightCard: React.FC<{ children: React.ReactNode; className?: string; onClick?: () => void; borderBeam?: boolean }> = ({ children, className = "", onClick, borderBeam }) => {
  ...
  return (
    ...
    >
      ...
      {/* Border Beam Effect for Featured Cards */}
      {borderBeam && (
...
const NavHeader: React.FC<{ onLaunch: () => void }> = ({ onLaunch }) => {
  ...
  return (
    <MotionNav 
        ...
        className="absolute top-0 left-0 right-0 z-50 border-b border-white/5 bg-[#050505]/50 backdrop-blur-xl h-16 flex items-center pt-safe cursor-interactive"
    >
        ...
        <div className="flex items-center gap-4 sm:gap-6">
            ...
            <button 
            onClick={onLaunch}
            className="bg-zinc-100 hover:bg-white text-black px-4 sm:px-5 py-2 rounded-sm text-[10px] font-bold uppercase tracking-[0.15em] transition-all hover:scale-105 active:scale-95 flex items-center gap-2 shadow-[0_0_20px_rgba(255,255,255,0.1)] hover:shadow-[0_0_30px_rgba(255,255,255,0.2)]"
            >
      ...
      <div className="relative z-10 text-center max-w-5xl mx-auto perspective-1000 pt-20 pointer-events-none w-full">
        ...
        >
            ...
            >
              <button 
                onClick={onLaunch}
                className="group relative w-full sm:w-auto px-8 py-4 bg-zinc-100 text-black rounded-sm font-bold text-xs uppercase tracking-[0.2em] hover:bg-white transition-all overflow-hidden shadow-[0_0_40px_-10px_rgba(255,255,255,0.3)] hover:shadow-[0_0_60px_-15px_rgba(255,255,255,0.5)] active:scale-95"
              >
              ...
              <button 
                onClick={onLaunch}
                className="w-full sm:w-auto px-8 py-4 bg-transparent border border-zinc-800 text-zinc-400 rounded-sm font-bold text-xs uppercase tracking-[0.2em] hover:border-zinc-600 hover:text-white transition-all font-mono flex items-center justify-center gap-2 hover:bg-zinc-900 cursor-interactive active:scale-95"
              >
...
const TrustStrip: React.FC = () => {
    ...
    return (
        <MotionSection 
            ...
            className="py-8 border-b border-white/5 bg-[#050505] relative z-20 overflow-hidden cursor-default"
        >
...
const ScrollNarrative: React.FC<{ onLaunch: () => void } & ScrollProps> = ({ onLaunch, scrollContainer }) => {
  ...
  // Beat Opacities (Condensed phases for better pacing)
  ...
  // Parallax
  const yParallax = useTransform(scrollYProgress, [0.6, 0.9], [100, -100]);
      ...
      <div className="sticky top-0 h-[100dvh] w-full overflow-hidden flex flex-col items-center justify-center">
        ...
        <MotionDiv style={{ opacity: op1 }} className="absolute inset-0 flex items-center justify-center pointer-events-none px-6">
           <div className="font-mono text-xl sm:text-3xl md:text-5xl font-bold text-white tracking-tighter text-center relative max-w-lg md:max-w-none bg-[#050505]/90 p-8 border border-white/5 rounded-xl shadow-2xl backdrop-blur-md">
              ...
              Goal: Build Fintech MVP
              <span className="animate-pulse ml-1 text-amber-500">_</span>
           </div>
        </MotionDiv>
        ...
        <MotionDiv style={{ opacity: op2 }} className="absolute inset-0 flex items-center justify-center pointer-events-none px-6">
            <div className="relative flex flex-col items-center w-full max-w-4xl">
                {/* Visual Tree */}
                <div className="absolute inset-0 w-full h-full pointer-events-none -top-24 md:-top-32 flex justify-center">
                    <svg className="w-full h-full max-w-2xl" viewBox="0 0 900 400" preserveAspectRatio="xMidYMid meet">
                        <defs>
                        ...
                        <g className="opacity-30">
                            <path d="M450 100 L450 180" stroke="white" strokeWidth="2" vectorEffect="non-scaling-stroke"/>
                            <path d="M450 180 L200 250" stroke="white" strokeWidth="2" vectorEffect="non-scaling-stroke"/>
                            <path d="M450 180 L450 250" stroke="white" strokeWidth="2" vectorEffect="non-scaling-stroke"/>
                            <path d="M450 180 L700 250" stroke="white" strokeWidth="2" vectorEffect="non-scaling-stroke"/>
                        </g>
                        {/* Animated Flow */}
                        <g className="animate-[pulse_2s_infinite]">
                            <circle r="4" fill="#f59e0b">
                                <animateMotion dur="1.5s" repeatCount="indefinite" path="M450 100 L450 180 L200 250" />
                            </circle>
                            <circle r="4" fill="#f59e0b">
                                <animateMotion dur="1.5s" begin="0.2s" repeatCount="indefinite" path="M450 100 L450 180 L450 250" />
                            </circle>
                            <circle r="4" fill="#f59e0b">
                                <animateMotion dur="1.5s" begin="0.4s" repeatCount="indefinite" path="M450 100 L450 180 L700 250" />
                            </circle>
                        </g>
                    </svg>
                </div>
        ...
        <MotionDiv style={{ opacity: op3 }} className="absolute inset-0 flex items-center justify-center pointer-events-none px-6">
            <MotionDiv style={{ y: yParallax }} className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6 w-full max-w-5xl">
        ...
        <MotionDiv style={{ opacity: op4 }} className="absolute inset-0 flex items-center justify-center pointer-events-none px-4">
             <div className="w-full max-w-3xl aspect-video bg-[#09090b] border border-zinc-800 rounded-lg shadow-2xl overflow-hidden font-mono text-xs p-4 md:p-6 text-zinc-400 ring-1 ring-white/10 relative">
                 ...
                 <div className="space-y-2 relative z-10">
                     ...
                     <div className="mt-4 p-3 bg-white/5 border border-white/5 rounded text-zinc-300 hidden sm:block">
                         System Status: NOMINAL<br/>
                         Outcome Verified. Infrastructure deployed. Code compiled.
                     </div>
                 </div>
             </div>
        </MotionDiv>
    ...
    <section className="py-24 bg-[#050505] border-t border-white/5 relative z-10 overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-6 relative z-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <SpotlightCard className="rounded-2xl p-8 md:p-10 min-h-[320px] md:h-[400px] h-auto flex flex-col justify-between">
                    <div>
                        ...
                        <p className="text-zinc-400 leading-relaxed text-sm md:text-base">Intelligence that remembers. Persistent vector memory automatically assimilates project constraints, user preferences, and factoids into a retrievable context window.</p>
                    </div>
                    <div className="text-xs font-mono text-zinc-500 bg-black/40 p-4 rounded border border-white/5 mt-6 md:mt-0">
                        <div className="flex gap-2 mb-2"><span className="text-purple-500">*</span> Retrieving context: "User prefers TypeScript"</div>
                        <div className="flex gap-2"><span className="text-emerald-500">✓</span> Context injection: ACTIVE</div>
                    </div>
                </SpotlightCard>

                <SpotlightCard className="rounded-2xl p-8 md:p-10 min-h-[320px] md:h-[400px] h-auto flex flex-col justify-between">
                    <div>
                        <div className="w-12 h-12 rounded-lg bg-zinc-900 border border-zinc-800 flex items-center justify-center mb-6 text-emerald-500">
                            <Server size={28} />
                        </div>
                        <h3 className="text-2xl font-bold text-white mb-2">Antigravity Ops</h3>
                        <p className="text-zinc-400 leading-relaxed text-sm md:text-base">DevOps without the Ops. Deploy infrastructure via conversation. Manage Docker, K8s, and cloud scaling without leaving the chat.</p>
                    </div>
                    <div className="text-xs font-mono text-zinc-500 bg-black/40 p-4 rounded border border-white/5 mt-6 md:mt-0">
                        {">"} antigravity deploy --container=luxor-v1<br/>
                        <span className="text-emerald-500">[SUCCESS]</span> App live at https://luxor.app
                    </div>
                </SpotlightCard>
            </div>
        </div>
...
const TerminalDemo: React.FC<{ onLaunch: () => void }> = ({ onLaunch }) => {
    const [lines, setLines] = useState<string[]>([
        "user@luxor:~$ awaiting_command..."
    ]);
    ...
    const runAudit = () => {
        if(isRunning) return;
        setIsRunning(true);
        setLines(["user@luxor:~$ run_system_audit"]);
        
        const sequence = [
            { text: "> Initiating deep scan...", delay: 600 },
            { text: "> Analyzing /var/www/logs...", delay: 1400 },
            { text: "[WARN] Latency spike detected in us-east-1 (240ms)", delay: 2200, color: "text-amber-500" },
            { text: "> Auto-scaling initiated.", delay: 3000 },
            { text: "[SUCCESS] Latency normalized.", delay: 4200, color: "text-emerald-500" },
            { text: "user@luxor:~$ ", delay: 5000 }
        ];
        ...
        sequence.forEach(({ text, delay, color }) => {
            setTimeout(() => {
                ...
                if (text.includes("user@")) setIsRunning(false);
            }, delay);
        });
    };
        ...
        <section className="py-24 bg-black border-t border-white/5 relative overflow-hidden">
             <div className="max-w-4xl mx-auto relative z-10 px-6">
                 <div className="text-center mb-12">
                     <h2 className="text-3xl font-bold text-transparent bg-clip-text bg-gradient-to-b from-white to-zinc-600 brand-font mb-4">
                        <DecryptText text="INTERACTIVE SHELL" animateOnView />
                     </h2>
                     <p className="text-zinc-500">From abstract intent to concrete execution in milliseconds.</p>
                 </div>
                 ...
                 <MotionDiv 
                    ...
                    className="bg-[#0c0c0e] rounded-xl border border-zinc-800 shadow-2xl overflow-hidden font-mono text-sm ring-1 ring-white/10 relative cursor-interactive active:cursor-grabbing max-w-full group"
                 >
                     {/* CRT Overlay Effects */}
                     ...
                     <div className="bg-zinc-900/80 p-3 flex items-center gap-2 border-b border-white/5 backdrop-blur-sm relative z-30">
                         <div className="flex gap-1.5">
                             <div className="w-3 h-3 rounded-full bg-red-500/20 border border-red-500/50"></div>
                             <div className="w-3 h-3 rounded-full bg-amber-500/20 border border-amber-500/50"></div>
                             <div className="w-3 h-3 rounded-full bg-emerald-500/20 border border-emerald-500/50"></div>
                         </div>
                         <div className="ml-auto text-zinc-600 text-xs">/bin/bash — 80x24</div>
                     </div>
                     
                     <div className="p-4 md:p-6 h-64 md:h-80 overflow-y-auto flex flex-col justify-end relative z-10" onClick={(e) => { e.stopPropagation(); runAudit(); }}>
                         {!isRunning && lines.length < 2 && (
                             <div className="absolute inset-0 flex items-center justify-center bg-black/50 backdrop-blur-[1px] cursor-pointer group/btn z-20">
                                 <button className="bg-white text-black px-6 py-3 rounded-sm font-bold text-xs uppercase tracking-widest hover:scale-105 transition-transform flex items-center gap-2 shadow-[0_0_20px_rgba(255,255,255,0.3)]">
                                     <Play size={12} fill="currentColor" /> Run System Audit
                                 </button>
                             </div>
                         )}
                         
                         {lines.map((line, i) => (
                             <div key={i} className="mb-1 text-zinc-300 break-words shadow-black drop-shadow-sm" dangerouslySetInnerHTML={{ __html: line }} />
                         ))}
                         
                         <div className="flex gap-2 mt-1 min-h-[20px]">
                             {isRunning ? (
                                <span className="animate-pulse bg-emerald-500 w-2 h-5 shadow-[0_0_8px_rgba(16,185,129,0.8)]"></span>
                             ) : (
                                <span className={`${cursorVisible ? 'opacity-100' : 'opacity-0'} w-2 h-5 bg-zinc-500`}></span>
                             )}
                         </div>
                         
                         {!isRunning && lines.length > 2 && (
                             <div className="absolute bottom-6 right-6 animate-in fade-in duration-500 z-30">
                                 <button onClick={(e) => { e.stopPropagation(); onLaunch(); }} className="px-4 py-2 bg-emerald-600 text-white font-bold text-xs uppercase tracking-widest rounded hover:bg-emerald-500 transition-colors shadow-lg cursor-interactive">
                                     Take Real Control
                                 </button>
                             </div>
                         )}
...
const PricingDeck: React.FC<{ onLaunch: () => void }> = ({ onLaunch }) => {
    return (
        <section className="py-24 bg-[#09090b] border-t border-white/5 px-6 relative pb-40">
            ...
            <div className="max-w-7xl mx-auto relative z-10">
                <div className="text-center mb-16">
                    <h2 className="text-3xl md:text-4xl font-bold text-transparent bg-clip-text bg-gradient-to-b from-white to-zinc-600 brand-font mb-4">
                        <DecryptText text="TRANSPARENT DEPLOYMENT" animateOnView />
                    </h2>
                    <p className="text-zinc-500 text-base">Bring your own keys. Pay for the orchestration.</p>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8 items-start">
                    {[
                        { name: "Operator", price: "$0", desc: "For individual builders.", features: ["BYO API Keys", "Local Memory", "Access to all Agents"] },
                        { name: "Syndicate", price: "$49", desc: "For small teams.", features: ["Cloud Memory Sync", "Shared Personas", "Priority Inference"], highlight: true },
                        { name: "Enterprise", price: "Custom", desc: "For organizations.", features: ["Private VPC Deployment", "SSO & Audit Logs", "Dedicated Architect"] }
                    ].map((plan, i) => (
                        <SpotlightCard 
                            key={i} 
                            borderBeam={plan.highlight}
                            className={`p-8 h-full flex flex-col rounded-2xl group transition-all duration-300 ${plan.highlight ? 'bg-zinc-900/40 shadow-2xl shadow-amber-900/10' : 'bg-zinc-900/10 hover:border-zinc-700'}`}
                        >
                            {plan.highlight && (
                                <div className="absolute top-0 right-0 bg-amber-500 text-black text-[10px] font-bold px-3 py-1 uppercase tracking-widest rounded-bl-xl shadow-lg z-20">
                                    Most Popular
                                </div>
                            )}
                            <h3 className="text-xl font-bold text-white mb-2 brand-font tracking-wide">{plan.name}</h3>
                            <div className="text-3xl font-bold text-white mb-1 font-mono">{plan.price}<span className="text-sm font-normal text-zinc-500">/mo</span></div>
                            ...
                            <div className="space-y-4 mb-8 flex-1">
                                {plan.features.map(f => (
                                    <div key={f} className="flex items-center gap-3 text-sm text-zinc-400">
                                        <CheckCircle2 size={16} className={plan.highlight ? "text-amber-500" : "text-zinc-700"} /> {f}
                                    </div>
                                ))}
                            </div>
                            ...
                            <button 
                               onClick={onLaunch}
                               className={`w-full py-4 rounded-sm font-bold text-[10px] uppercase tracking-[0.2em] transition-all relative z-20 cursor-interactive active:scale-95 ${plan.highlight ? 'bg-amber-500 text-black hover:bg-amber-400 shadow-lg' : 'bg-white/5 text-white hover:bg-white/10 border border-white/10'}`}
                            >
                                {plan.name === 'Enterprise' ? 'Request Access' : 'Initialize'}
                            </button>
                        </SpotlightCard>
                    ))}
                </div>
...
const LandingPage: React.FC<Props> = ({ onLaunch, isLaunching }) => {
  ...
  return (
    <div className={`fixed inset-0 z-50 bg-[#050505] text-zinc-300 font-sans selection:bg-amber-500/30 transition-all duration-1000 ease-in-out ${isLaunching ? 'opacity-0 scale-105 filter blur-xl' : 'opacity-100'}`}>
      <CustomCursor />
      ...
      >
          ...
          <FeatureDeepDive />
          <TerminalDemo onLaunch={onLaunch} />
          <PricingDeck onLaunch={onLaunch} />
          
          <footer className="py-20 px-6 border-t border-white/5 bg-[#050505] relative z-20 cursor-default">
              <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-8">
                  <div className="flex flex-col gap-4 text-center md:text-left">
                     <div className="flex items-center gap-2 justify-center md:justify-start">
                        <div className="w-6 h-6 rounded bg-zinc-800 flex items-center justify-center font-bold text-white text-[10px]">L9</div>
                        <span className="font-bold text-zinc-200 text-sm tracking-widest brand-font">LUXOR9</span>
                     </div>
                     <p className="text-zinc-500 text-xs max-w-xs">System V3.2 Online. The infrastructure is primed. The agents are listening.</p>
                  </div>
                  <div className="flex gap-8 text-[10px] font-mono text-zinc-500 uppercase tracking-widest">
                      <button className="hover:text-white transition-colors cursor-interactive">Documentation</button>
                      <button className="hover:text-white transition-colors cursor-interactive">Security</button>
                      <button className="hover:text-white transition-colors cursor-interactive">API Status</button>
                  </div>
              </div>
          </footer>
      </div>
