.PHONY: setup install install-dev install-gpu test test-contracts lint clean clean-projects preflight demo demo-list hyperframes-doctor hyperframes-warm remix-setup remix-check remix-demo

# Pinned HyperFrames npm version — shared by setup/warm targets and CI.
# Update deliberately (check upstream changelog); runtime can override via
# the LUXOR9_HYPERFRAMES_VERSION environment variable.
...
setup:
	...
	@npx --yes hyperframes@$(HYPERFRAMES_VERSION) --version >/dev/null 2>&1 && echo "    HyperFrames CLI cached (npx, v$(HYPERFRAMES_VERSION))" || echo "  [skip] HyperFrames cache-warm failed — offline or npm unavailable; first render will fetch on demand"
	...
	@echo "  Optional: run 'make hyperframes-warm' anytime to refresh the npx cache to the pinned hyperframes version."
...
hyperframes-warm:
	@echo "==> Refreshing the HyperFrames npx cache (pinned v$(HYPERFRAMES_VERSION))..."
	@echo "    Version is pinned in the Makefile (HYPERFRAMES_VERSION) so CI and"
	@echo "    local runs resolve the exact same runtime. Bump it deliberately."
	npx --yes --prefer-online hyperframes@$(HYPERFRAMES_VERSION) --version
...
remix-setup:
	...
	@npx --yes hyperframes@$(HYPERFRAMES_VERSION) --version > /dev/null 2>&1 && echo "    OK" || echo "    SKIP (will fetch on first render)"
	@echo ""
...
clean-projects:
	@echo "==> Pruning projects/ older than $${DAYS:-30} idle days (dry-run unless APPLY=1)..."
ifdef APPLY
	python scripts/cleanup_projects.py --days $${DAYS:-30} --apply
else
	python scripts/cleanup_projects.py --days $${DAYS:-30}
endif
